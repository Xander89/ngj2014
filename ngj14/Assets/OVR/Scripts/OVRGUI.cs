...
public class OVRGUI
{
	...
	// StereoDrawSprite
	public void StereoDrawSprite(int X, int Y, int wX, int hY, ref Sprite image, Color color)
	{
		GUI.color = color;
		// Make sure to change font if it needs replacement
		if(GUI.skin.font != FontReplace) GUI.skin.font = FontReplace;
		...
		//GUI.DrawSprite(DrawRect, image);
	}
	public void StereoDrawSprite(float X, float Y, float wX, float hY, ref Sprite image, Color color)
	{
		StereoDrawSprite ((int)(X  * PixelWidth), 
		                   (int)(Y  * PixelHeight),
		                   (int)(wX * PixelWidth),
		                   (int)(hY * PixelHeight),
		                   ref image, color);
